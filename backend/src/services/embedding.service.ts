// Embedding Service — Dual-Engine Production Architecture
// Engine 1: Remote HF API (0 MB Server RAM — prevents 512MB RAM Render crashes)
// Engine 2: Local ONNX Engine (Fallback with small batch size=4 & event-loop yielding)
...
async function generateRemoteEmbeddings(texts: string[]): Promise<number[][] | null> {
  try {
    ...
    return parseHfResponse(data, texts.length);
  } catch {
    return null;
  }
}
...
function parseHfResponse(data: any, expectedCount: number): number[][] | null {
  ...
  // 1. Direct array of vectors: [[384 numbers], ...]
